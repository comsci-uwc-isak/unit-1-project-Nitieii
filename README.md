...
| Easy commands that allow to enter the detail of the trip distance (km) | Yes | The user run the program, and just need to enter the detail of trip |
| A basic permanent storage system and backup  | Yes | Enter the location and the db folder will be backed up into that location |
| A command to see total statistic | Yes | The user just need to enter the name of car or all to show statistic of that car or all the cars |
| User-skill independent | Yes | Every prgram is easy to use with two or less steps |
